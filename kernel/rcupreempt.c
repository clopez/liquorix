...
static int rcu_sched_qs_needed_dyntick(int cpu)
{
	long curr;
	long snap;
	struct rcu_dyntick_sched *rdssp = &per_cpu(rcu_dyntick_sched, cpu);
...
# define rcu_sched_qs_needed_dyntick(cpu)		(1)
...
static inline int rcu_sched_qs_needed(int cpu)
{
	struct rcu_dyntick_sched *rdssp = &per_cpu(rcu_dyntick_sched, cpu);
...
static int rcu_sched_grace_period(void *arg)
{
	...
	do {
		 ...
		 */
		for_each_online_cpu(cpu) {
			while (rcu_sched_qs_needed(cpu) &&
			       rcu_sched_qs_needed_dyntick(cpu)) {
				/* resched_cpu(cpu); @@@ */
